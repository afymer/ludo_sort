...
const BUDGET: f32 = 150.;

const NBR_JEUX_MAX: usize = 12;
// const NBR_JEUX_MAX: usize = 8;
// const NBR_JEUX_MAX: usize = 4;
        ...
        for (user_index, user) in notes.iter().enumerate() {
            if let Some(note) = user[**i] {
                if note > means[user_index] {
                    likings[user_index].1 += 1. / ALPHA.powf(likings[user_index].0 as f32);
                    likings[user_index].0 += 1;
                }
            }
        }
    }
    likings.iter().fold(0., |acc, (_, liking)| acc + liking)
}

fn budget(games: &[Game<'_>], selection: &[&usize]) -> f32 {
    selection
        .iter()
        .fold(0., |x, &&index| x + games[index].price)
}
...
fn main() {
    ...
    let means: Vec<f32> = notes
        .iter()
        .map(|l| {
            let (sum, count) = l.iter().fold((0., 0.), |(acc_val, acc_sum), c| {
                if let Some(v) = c {
                    (acc_val + v, acc_sum + 1.)
                } else {
                    (acc_val, acc_sum)
    ...
    for nbr in 0..NBR_JEUX_MAX {
        ...
        {
            let price = budget(&games, &combination);
            if price > BUDGET {
                continue;
            }
